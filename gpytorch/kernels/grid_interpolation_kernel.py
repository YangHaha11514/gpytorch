        ...
        if d > 1:
            ...
            for i in range(d):
                ...
                if both_min < self.grid_bounds[i][0] or both_max > self.grid_bounds[i][1]:
                    ...
                    if torch.abs(both_min - self.grid[i][0].data)[0] > 1e-7 or torch.abs(both_max - self.grid[i][-1].data)[0] > 1e-7:
                        raise RuntimeError('Received data that was out of bounds for the specified grid. \
                                            Grid bounds were ({}, {}), but min = {}, max = {}'.format(self.grid_bounds[i][0],
